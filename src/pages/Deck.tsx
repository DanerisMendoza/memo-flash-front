...
import themeDeck from "../theme/index.tsx";
...
export default function Inventory() {
  ...
  const REVIEW_DIALOG = useSelector(
    (state: RootState) => state.cardReducer.REVIEW_DIALOG
  );
  ...
  return (
    <div className="h-screen w-screen flex flex-col p-8 mt-4">
      <ThemeProvider theme={themeDeck}>
        <Card className="mt-4">
          <CardContent>
              ...
              DECKS.map((deck: DeckInterface, index) => (
                <Card key={index} className="mt-4">
                  <CardContent className="flex gap-2">
                    ...
                    <div className="grow"></div>
                    <Button
                      sx={{background:'green', opacity:'0.8'}}
                      variant="contained"
                      color="secondary"
                      onClick={() => {
                        reviewDeck(deck);
                    ...
                    <Button variant="outlined" onClick={() => openDeck(deck)}>
                      <EditIcon />
                    </Button>
                    <Button
                      variant="contained"
                      onClick={() => deleteSelectedDeck(deck)}
